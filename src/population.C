void population()
{
  ...
  TH1D* frame = new TH1D("frame", "Population by Age;Age;Population per 5-year Generation (%/5 year)", kBinsN, 0, 105);
  frame->SetMaximum(10);
  frame->Draw();
  ...
  for(Int_t i = 0; i < kHistN; i++){
    graph[i] = new TGraph();
    for(Int_t j = 0; j < kBinsN; j++){
      graph[i]->SetPoint(j, hist[i]->GetBinCenter(j + 1), hist[i]->GetBinContent(j + 1));
    } // j
